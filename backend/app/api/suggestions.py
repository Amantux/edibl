...
from ..models import AiSuggestion, Product
...
def _member_names(ids, gid) -> dict:
    if not ids:
        return {}
    rows = (db.session.query(Product.id, Product.name)
            .filter(Product.group_id == gid, Product.id.in_(ids)).all())
...
def _out(s, names=None) -> dict:
    ...
    if s.kind == "categorize":
        d["product"] = ({"id": s.product.id, "name": s.product.name,
                         "category": s.product.category} if s.product else None)
    else:
        ids = (s.payload or {}).get("itemIds", [])
...
def list_suggestions():
    ...
    q = (db.session.query(AiSuggestion)
         .options(selectinload(AiSuggestion.product))
    ...
    ids = [i for s in rows if s.kind == "cluster" for i in (s.payload or {}).get("itemIds", [])]
