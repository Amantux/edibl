...
_LOGGER = logging.getLogger("edibl.db_copy")
...
def _order_for_self_fk(table, rows):
    ...
    locations.parent_id -> locations.id). Postgres enforces FKs per row on insert,
    so a child stored physically before its parent would be rejected. No-op for
    tables without a self-FK. Cross-table FK order is handled by sorted_tables."""
