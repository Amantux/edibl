"""The single configuration contract for Edibl.
...
``EDIBL_`` value — the inventory, the docs, the ``config_check`` CLI and the CI
...
# ends up with registration open because someone wrote "EDIBL_ALLOW_REGISTRATION=maybe".
# --------------------------------------------------------------------------
...
def provider_choice(*allowed: str) -> Callable[[str], str]:
    ...
    provider name at the point of use (``Config.LLM_PROVIDER`` did
    ``.strip().lower()``), so
    ``EDIBL_LLM_PROVIDER=Ollama`` works today. A strict parser would turn a
...
def normalize_db_url(url: str) -> str:
    ...
    Edibl supports exactly two backends: SQLite and PostgreSQL via psycopg
    ...
    if base == "postgresql":
        ...
        if driver != "psycopg":
            raise ValueError(
                f"unsupported Postgres driver '+{driver}'. Edibl ships psycopg "
...
class Field:
    name: str                      # without the EDIBL_ prefix
    ...
    supports_file: bool = False    # honours EDIBL_<NAME>_FILE (Docker secrets)

    @property
    def env_var(self) -> str:
        return f"EDIBL_{self.name}"
...
FIELDS: tuple[Field, ...] = (
    ...
    Field("SECRET_KEY", as_str, "",
          ...
          "Deliberately has no add-on option: a key that changed per boot would "
          "log every user out and void every API token on each restart.",
    ...
    Field("SEED_DEFAULTS", parse_bool, True,
          "Seed a new database with the default locations, units and categories."),

    # --- network / proxy ---
    Field("PORT", int_between(1, 65535), 7746, "HTTP port for the app."),
    ...
    # --- LLM assistant ---
    Field("LLM_PROVIDER", provider_choice("", "openai", "ollama", "claude", "homeassistant"),
          "", "Which LLM backend the assistant uses. Blank = rules only.",
          ha_option="llm_provider"),
    Field("LLM_BASE_URL", as_str, "",
          "Base URL of the LLM server (local Ollama, an OpenAI-compatible "
          "gateway, or blank for the provider default).",
          ha_option="llm_base_url"),
    Field("LLM_API_KEY", as_str, "", "API key for the configured LLM provider.",
          secret=True, supports_file=True, ha_option="llm_api_key"),
    Field("LLM_MODEL", as_str, "", "Model name for the configured LLM provider.",
          ha_option="llm_model"),
    Field("LLM_AGENT_ID", as_str, "",
          ...
          ha_option="llm_agent_id"),
    Field("LLM_TIMEOUT", int_between(1, 600), 60,
          "Per-request timeout for LLM provider calls, in seconds."),
    Field("LLM_MAX_STEPS", int_between(1, 50), 6,
          "Maximum tool-calling rounds the assistant may take per message."),
    Field("AI_CONFIDENCE_THRESHOLD", float_between(0.0, 1.0), 0.8,
          "Auto-categorization: a proposal at/above this model-reported "
          "confidence is applied automatically; below it goes to review."),
    ...
    Field("BARCODE_LOOKUP", parse_bool, False,
          "Look an unknown scanned barcode up in Open Food Facts. Off by "
          "default because it makes outbound network calls.",
    ...
    # --- companion apps ---
    Field("MYMEAL_URL", as_str, "",
          "Base URL of a companion myMeal instance. Blank disables the "
          "integration. Also settable in the UI, which is remembered."),
    Field("MYMEAL_TOKEN", as_str, "",
          "API token Edibl presents to myMeal.", secret=True, supports_file=True),
    Field("HOMEHOARD_URL", as_str, "",
          "Base URL of a companion HomeHoard instance. Blank disables it."),
    Field("HOMEHOARD_TOKEN", as_str, "",
          "API token Edibl presents to HomeHoard.", secret=True, supports_file=True),
    ...
    Field("MCP_ENABLED", parse_bool, True,
          ...
          ha_option="mcp_enabled"),
    Field("MCP_HOST", as_str, "0.0.0.0", "Bind address for the MCP server."),
    Field("MCP_PORT", int_between(1, 65535), 7767, "Port for the MCP server (SSE)."),
    ...
    Field("MCP_SERVER_TOKEN", as_str, "",
          ...
          secret=True, supports_file=True, ha_option="mcp_server_token"),
...
class Settings:
    ...
    def sqlalchemy_uri(self) -> str:
        ...
        if explicit:
            ...
            try:
                ...
                raise ConfigError([f"EDIBL_DATABASE_URL: {exc}"]) from None
        ...
        if self.values["USE_SHARED_POSTGRES"]:
            try:
                ...
                pass
        return f"sqlite:///{os.path.join(self.data_dir, 'edibl.db')}"
    ...
    def ai_enabled(self) -> bool:
        """Whether the LLM assistant is configured. Blank provider = rules-only,
        which is a supported mode, not a misconfiguration."""
        return bool(self.values["LLM_PROVIDER"])
...
def _validate_semantics(values, sources, in_ha, errors, warnings) -> None:
    ...
    if secret and not values["DISABLE_AUTH"]:
        if _is_placeholder(secret):
            errors.append(
                "EDIBL_SECRET_KEY is a known placeholder value. Generate one with: "
                "python3 -c 'import secrets;print(secrets.token_urlsafe(32))'"
            )
        elif len(secret) < MIN_SECRET_LENGTH:
            errors.append(
                f"EDIBL_SECRET_KEY is only {len(secret)} characters; at least "
    ...
    if values["DISABLE_AUTH"]:
        ...
        if not in_ha and values["PROXY_HOPS"] == 0:
            warnings.append(
                ...
                "is only safe if something in front of Edibl authenticates "
                "callers — set EDIBL_PROXY_HOPS to confirm a proxy is present."
            )
        if values["CORS_ORIGINS"]:
            errors.append(
                "DISABLE_AUTH=true together with EDIBL_CORS_ORIGINS is unsafe: it lets "
    ...
    if values["DEBUG"]:
        if in_ha:
            errors.append("EDIBL_DEBUG cannot be enabled in the Home Assistant add-on.")
        else:
            warnings.append(
                "EDIBL_DEBUG is on. The Werkzeug debugger executes arbitrary code — "
    ...
    if "*" in values["CORS_ORIGINS"]:
        errors.append(
            "EDIBL_CORS_ORIGINS may not contain '*': credentialed requests from any "
            "origin would be permitted. List explicit origins instead."
        )
    for origin in values["CORS_ORIGINS"]:
        if not re.match(r"^https?://", origin):
            errors.append(
                f"EDIBL_CORS_ORIGINS entry {origin!r} must start with http:// or https://")

    # --- LLM coherence. Warnings only: the assistant is optional, and a
    # missing key must never stop the pantry app from serving. ---
    provider = values["LLM_PROVIDER"]
    if provider in ("openai", "claude") and not values["LLM_API_KEY"]:
        warnings.append(
            f"LLM_PROVIDER={provider} but EDIBL_LLM_API_KEY is not set. The "
            "assistant will fall back to rules-only; the rest of the app is "
            "unaffected."
        )
    if provider == "homeassistant" and values["LLM_BASE_URL"]:
        warnings.append(
            "LLM_PROVIDER=homeassistant ignores EDIBL_LLM_BASE_URL — the agent is "
            "reached through the Supervisor, not a URL."
        )
    if values["LLM_AGENT_ID"] and provider != "homeassistant":
        warnings.append(
            "EDIBL_LLM_AGENT_ID only applies to the homeassistant provider and is "
            f"inert with LLM_PROVIDER={provider or 'unset'}."
        )

    # A base URL that isn't a URL is a warning, not a fatal: the assistant is
    # optional, and taking the whole pantry app down for it is the worse outcome.
    base_url = values["LLM_BASE_URL"]
    if base_url and not re.match(r"^https?://", base_url):
        warnings.append(
            f"EDIBL_LLM_BASE_URL={base_url!r} does not start with http:// or "
            "https://; the assistant will fail to connect."
        )
    ...
    if uri:
        try:
            ...
            errors.append(f"EDIBL_DATABASE_URL: {exc}")
    if values["MIGRATE_FROM_SQLITE"] and not uri and not values["USE_SHARED_POSTGRES"]:
        warnings.append(
            ...
            "(EDIBL_DATABASE_URL is blank and USE_SHARED_POSTGRES is off), so there "
    ...
    if values["MCP_ENABLED"] and values["MCP_PORT"] == values["PORT"]:
        errors.append(
            f"EDIBL_MCP_PORT and EDIBL_PORT are both {values['PORT']}; "
...
def ensure_secret_key(supplied: str, data_dir: str) -> tuple[str, bool]:
    ...
    rather than a config problem. The entrypoint used to default EDIBL_SECRET_KEY
