...
``reap_stale`` requeues. Disabled under tests (``WORKER_ENABLED=False``).
"""
...
_LOGGER = logging.getLogger("edibl.worker")
...
def start_worker(app) -> None:
    ...
    threading.Thread(target=_loop, args=(app,), name="edibl-job-worker",
                     daemon=True).start()
