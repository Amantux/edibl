...
CONFIG_YAML = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "..", "addon", "config.yaml"))
...
def test_parse_bool_unknown_value_raises_rather_than_defaulting_false():
    ...
    EDIBL_ALLOW_REGISTRATION=maybe quietly turned registration off."""
...
def test_load_settings_invalid_bool_names_the_variable_and_accepted_values():
    with pytest.raises(ConfigError) as exc:
        resolve(env={"EDIBL_ALLOW_REGISTRATION": "maybe"})

    (msg,) = exc.value.errors
    assert "EDIBL_ALLOW_REGISTRATION" in msg
    assert "'maybe'" in msg
...
def test_load_settings_non_integer_port_is_rejected():
    with pytest.raises(ConfigError) as exc:
        resolve(env={"EDIBL_PORT": "not-a-port"})

    assert "EDIBL_PORT" in exc.value.errors[0]
...
def test_load_settings_reports_every_error_at_once():
    ...
    with pytest.raises(ConfigError) as exc:
        resolve(env={
            "EDIBL_PORT": "banana",
            "EDIBL_DISABLE_AUTH": "perhaps",
            "EDIBL_AI_CONFIDENCE_THRESHOLD": "9",
        })
...
def test_load_settings_unknown_ai_provider_is_rejected():
    with pytest.raises(ConfigError):
        resolve(env={"EDIBL_LLM_PROVIDER": "gpt5-turbo-max"})


def test_load_settings_llm_provider_is_case_insensitive():
    """Config.LLM_PROVIDER did `.strip().lower()`, so EDIBL_LLM_PROVIDER=Ollama
    works today. A strict parser would refuse to boot a working deployment."""
    assert resolve(env={"EDIBL_LLM_PROVIDER": "Ollama"}).LLM_PROVIDER == "ollama"
    assert provider_choice("", "ollama")(" OLLAMA ") == "ollama"


def test_load_settings_blank_llm_provider_disables_the_assistant():
    s = resolve(env={"EDIBL_LLM_PROVIDER": ""})
...
def test_load_settings_bad_database_url_refuses_to_start():
    with pytest.raises(ConfigError) as exc:
        resolve(env={"EDIBL_DATABASE_URL": "mysql://u:p@h/db"})

    assert "EDIBL_DATABASE_URL" in exc.value.errors[0]
...
def test_load_settings_database_url_password_is_not_echoed_in_the_error():
    ...
    with pytest.raises(ConfigError) as exc:
        resolve(env={"EDIBL_DATABASE_URL": "mysql://user:hunter2@h/db"})
...
def test_load_settings_ha_option_beats_env():
    ...
    s = resolve(env={"EDIBL_DISABLE_AUTH": "false"}, ha_options={"disable_auth": True})
...
def test_load_settings_override_beats_everything():
    s = resolve(env={"EDIBL_PORT": "9000"}, overrides={"PORT": 1234})
...
def test_load_settings_empty_ha_option_wins_when_the_default_is_blank():
    s = resolve(env={"EDIBL_BARCODE_DB_KEY": "from-env"},
...
def test_load_settings_reports_the_source_of_every_value():
    s = resolve(env={"EDIBL_PORT": "9000"}, ha_options={"disable_auth": True},
...
def test_allow_registration_has_no_addon_option():
    """Unlike HomeHoard, Edibl does not expose registration as an add-on option,
    so there is nothing for an options.json to flip and no ha_default needed."""
    assert FIELDS_BY_NAME["ALLOW_REGISTRATION"].ha_option is None
...
def test_validate_false_skips_cross_field_rules_only():
    ...
    env = {"EDIBL_CORS_ORIGINS": "example.com", "EDIBL_DATABASE_URL": "sqlite:///x.db"}
...
def test_a_string_false_in_options_json_is_parsed_as_false():
    """The deleted addon/run.sh `getb` used `bool(v)`, so ANY non-empty string
    was true — `disable_auth: "false"` in a hand-edited options.json silently
    DISABLED authentication. Verified against the real helper before removing it.
    """
    s = resolve(ha_options={"disable_auth": "false"})

    assert s.DISABLE_AUTH is False


@pytest.mark.parametrize("value", ["no", "off", "0", "FALSE"])
def test_other_false_spellings_are_also_honoured(value):
    assert resolve(ha_options={"disable_auth": value}).DISABLE_AUTH is False


def test_a_nonsense_boolean_in_options_json_is_refused_not_coerced():
    """`getb` turned anything unrecognised into a silent true/false."""
    with pytest.raises(ConfigError):
        resolve(ha_options={"disable_auth": "sometimes"})


def test_absent_disable_auth_fails_closed():
    """`getb` returned "false" for a MISSING key (bool(None)), which happened to
    fail closed — keep that, explicitly, rather than by accident."""
    assert resolve(ha_options={"mcp_enabled": True}).DISABLE_AUTH is False


def test_absent_mcp_enabled_follows_the_manifest_default():
    """addon/config.yaml ships `mcp_enabled: true`, but `getb` returned "false"
    for a missing key — so a partial options.json silently turned voice control
    off. The declared default now wins.
    """
    assert resolve(ha_options={"disable_auth": True}).MCP_ENABLED is True
...
def test_load_settings_placeholder_secret_is_refused():
    with pytest.raises(ConfigError) as exc:
        resolve(env={"EDIBL_SECRET_KEY": "change-me-in-production"})
...
def test_load_settings_long_placeholder_secret_is_still_refused():
    ...
    with pytest.raises(ConfigError):
        resolve(env={"EDIBL_SECRET_KEY": "please-change-me-to-a-long-random-string"})
...
def test_load_settings_short_secret_is_refused_inside_ha_too_when_auth_is_on():
    ...
    """
    with pytest.raises(ConfigError):
        resolve(env={"EDIBL_SECRET_KEY": "short"},
...
def test_load_settings_reads_a_secret_from_its_file_variant(tmp_path):
    ...
    s = resolve(env={"EDIBL_SECRET_KEY_FILE": str(path)})
...
def test_load_settings_unreadable_secret_file_is_an_error(tmp_path):
    with pytest.raises(ConfigError) as exc:
        resolve(env={"EDIBL_SECRET_KEY_FILE": str(tmp_path / "missing")})

    assert "secret file" in exc.value.errors[0]


def test_redacted_replaces_every_populated_secret():
    s = resolve(env={"EDIBL_SECRET_KEY": GOOD_SECRET,
                     "EDIBL_BARCODE_DB_KEY": "sk-very-secret"})
...
def test_redacted_leaves_blank_secrets_blank():
    ...
    assert resolve().redacted()["LLM_API_KEY"] == ""
...
def test_redacted_strips_credentials_embedded_in_a_url():
    ...
    into public issues. The name-suffix check above cannot catch this: LLM_BASE_URL
    ends in neither _KEY nor _TOKEN.
    """
    s = resolve(env={"EDIBL_LLM_BASE_URL": "http://user:s3cr3t@ollama.lan:11434"})

    shown = s.redacted()["LLM_BASE_URL"]
...
def test_load_settings_disable_auth_with_cors_origins_is_fatal():
    with pytest.raises(ConfigError) as exc:
        resolve(env={"EDIBL_DISABLE_AUTH": "true",
                     "EDIBL_CORS_ORIGINS": "https://evil.example"})
...
def test_load_settings_provider_without_its_key_warns_but_boots():
    s = resolve(env={"EDIBL_LLM_PROVIDER": "claude"})

    assert any("LLM_API_KEY" in w for w in s.warnings)


def test_load_settings_llm_base_url_without_a_scheme_warns_but_boots():
    """The assistant is optional — a bad URL must not take the pantry down."""
    s = resolve(env={"EDIBL_LLM_BASE_URL": "localhost:11434"})

    assert any("EDIBL_LLM_BASE_URL" in w for w in s.warnings)


def test_load_settings_migrate_from_sqlite_without_a_target_warns():
    s = resolve(env={"EDIBL_MIGRATE_FROM_SQLITE": "true"})
...
def test_data_dir_is_always_absolute():
    assert os.path.isabs(resolve(env={"EDIBL_DATA_DIR": "./rel"}).data_dir)


def test_sqlalchemy_uri_defaults_to_sqlite_inside_the_data_dir(tmp_path):
    s = resolve(env={"EDIBL_DATA_DIR": str(tmp_path)})

    assert s.sqlalchemy_uri == f"sqlite:///{tmp_path / 'edibl.db'}"


def test_sqlalchemy_uri_normalizes_an_explicit_database_url():
    s = resolve(env={"EDIBL_DATABASE_URL": "postgresql://u:p@h/db"})
...
def test_sqlalchemy_uri_uses_the_provisioned_dsn_for_shared_postgres(tmp_path):
    ...
    s = resolve(env={"EDIBL_DATA_DIR": str(tmp_path), "EDIBL_USE_SHARED_POSTGRES": "true"})
...
def test_mcp_api_is_derived_from_the_app_port_when_unset():
    assert resolve(env={"EDIBL_PORT": "9000"}).mcp_api == "http://127.0.0.1:9000/api/v1"


def test_derived_scalars_match_the_old_config_computations():
    s = resolve(env={"EDIBL_JWT_HOURS": "5", "EDIBL_MAX_UPLOAD_MB": "3"})
...
def test_resolution_creates_no_directories(tmp_path):
    ...
    s = resolve(env={"EDIBL_DATA_DIR": str(target)})
    _ = s.data_dir, s.sqlalchemy_uri
...
def test_two_apps_with_different_settings_can_coexist_in_one_process():
    ...
    a = resolve(env={"EDIBL_PORT": "1111"})
    b = resolve(env={"EDIBL_PORT": "2222"})
