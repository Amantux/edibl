...
from app.models import Group, Location, Product, User
...
def _seed(app):
    with app.app_context():
        ...
        db.session.add(User(name="Alex", email="a@x.com", password_hash="x",
                            is_owner=True, group_id=g.id))
        db.session.add(Product(name="Milk", group_id=g.id))
        db.session.commit()
...
def test_copy_preserves_all_tables_ids_and_counts(app, tmp_path):
    ...
    # Every model table is represented in the report (nothing silently skipped).
...
def test_order_for_self_fk_puts_parents_first():
    # Child listed before its parent in the input → parent must come out first,
    # or Postgres's per-row FK check rejects the child.
...
def test_copy_preserves_nested_locations(app, tmp_path):
    ...
    try:
        with app.app_context():
            ...
            child = Location(name="Fridge", kind="fridge", group_id=g.id)
            db.session.add(child)
            db.session.flush()
            parent = Location(name="Kitchen", kind="room", group_id=g.id)
        ...
        assert links["Fridge"] == parent_id  # child still points at its parent
        eng.dispose()
    finally:
...
def test_copy_refuses_non_empty_target(app, tmp_path):
    ...
    copy_database(app.config["SQLALCHEMY_DATABASE_URI"], target)  # first copy fills it
...
def test_migrate_policy_rejects_bad_endpoints(app):
    # source must be sqlite; target must be postgres.
...
def test_endpoint_forbidden_for_non_owner(app):
    # A logged-in member (non-owner) must be 403 — migration is owner-only.
    with app.app_context():
        g = Group(name="H")
        db.session.add(g)
        db.session.flush()
...
# --- boot-time auto-migrate guards (never crash startup) --------------------

def test_boot_migrate_noops_when_disabled(app):
    with app.app_context():
        app.config["MIGRATE_FROM_SQLITE"] = False
        _maybe_boot_migrate(app)  # must not raise
...
def test_boot_migrate_skips_without_source_file(app, tmp_path):
    with app.app_context():
        ...
        app.config["DATA_DIR"] = str(tmp_path)  # no edibl.db here → skip before connecting
...
def test_boot_migrate_raises_on_failure(app, tmp_path, monkeypatch):
    ...
    (tmp_path / "edibl.db").write_bytes(b"x")  # source present
    ...
    with app.app_context():
        ...
        with pytest.raises(RuntimeError, match="refusing to start"):
            _maybe_boot_migrate(app)  # must not raise / must not attempt a connection
