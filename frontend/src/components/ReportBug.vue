...
<template>
  <div class="modal-backdrop" @click.self="close">
    <div class="card modal" v-trap="close" aria-label="Report a bug">
      <div class="row" style="align-items:center">
        <h2 style="flex:1;margin:0">🐛 Report a bug</h2>
        <button class="ghost sm" aria-label="Close" @click="close">✕</button>
      </div>

      <div class="seg" role="group" aria-label="Report type" style="margin:14px 0">
        <button v-for="t in TYPES" :key="t.id" :class="{ on: type === t.id }"
      ...
      <div class="card" style="padding:11px;margin:0;background:var(--surface-2)">
        ...
        <p class="muted" style="font-size:.8rem;margin:8px 0 6px">
        ...
        <p v-if="loading" class="muted" style="font-size:.8rem;margin:6px 0 0">Gathering diagnostics…</p>
      </div>
