    ...
    <div v-for="l in locs" :key="l.id" class="card loc-card" style="margin:0"
      ...
      <div style="margin-top:8px" @click.stop>
        <input v-model="l.description" placeholder="What this area normally stores…"
          style="width:100%;font-size:.8rem;padding:6px 8px" aria-label="Area description"
          @change="saveDesc(l)" @keyup.enter="saveDesc(l)" />
        <div class="row" style="justify-content:flex-end;margin-top:4px">
          <button class="ghost sm" style="font-size:.72rem;padding:2px 6px" :disabled="genBusy === l.id"
            :aria-label="`Generate description for ${l.name}`"
            title="Generate from what's in it" @click="generateDesc(l)">✨ Generate</button></div></div>
    </div>
  </div>
